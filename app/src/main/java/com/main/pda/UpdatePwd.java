...
public class UpdatePwd extends Activity implements OnTouchListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// 重试单击事件
		clickupdate = new OnClickListener() {
			@Override
			public void onClick(View arg0) {
				managerClass.getAbnormal().remove();
				// 修改密码
				SystemUpdatePwdBiz.updatePwd(uid.getText().toString(), pwd.getText().toString(),
		...
		if (MotionEvent.ACTION_UP == even.getAction()) {
			...
			case R.id.btnlgoin:
				...
				if (regCheck(newpwd.getText().toString())) {
					...
					managerClass.getRuning().runding(UpdatePwd.this, "正在修改密码...");
					// 修改密码
					SystemUpdatePwdBiz.updatePwd(GApplication.loginname, pwd.getText().toString(),
							newpwd.getText().toString());
				} else {
					managerClass.getSureCancel().makeSuerCancel(UpdatePwd.this, "密码必须由数字和英文字母组成",
							new OnClickListener() {
								@Override
								public void onClick(View arg0) {
