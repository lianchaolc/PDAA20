...
//		enableIO = GPIO.getGPIO(GlobalConstant.IO_RFID_ENABLE);
	 ...
	 * 打开 RFID 串口
	 */
	private void setRFIDSerialPort() {
		for (int i = 0; i < 2; i++) {
			File rFidFile = new File(DeviceInfo.MODULE_RFID_FILE);
			try {
				this.rfidSerialPort = new SerialPort(rFidFile, DeviceInfo.MODULE_RFID_KEY, 0);
			} catch (SecurityException e) {
				e.printStackTrace();
			} catch (IOException e) {
				e.printStackTrace();
			}
			if (i == 0 && rfidSerialPort != null) {
	...
	public Boolean openRFID(Context context) {
		if (rfidSerialPort == null)
			setRFIDSerialPort();
		Log.d("liu_rui", "openRFID: RFIDDevice" + rfidSerialPort);
		if (rfidSerialPort == null)
			return false;
...
//				enableIO.setGPIO(GlobalConstant.ENABLE_IO);
				...
				System.out.println("RFIDDevice------------>openRfid()-------->start");
				try {
					Thread.sleep(200);
					// lanp----------------------->初始化设置信息
					if (count < 3) {
	...
	Boolean closeRFID() {
		Log.d("liu_rui", "closeRFID: RFIDDevice");
...
//			msg.what = ScanWriteActivity.readRFID;
			...
			System.out.println(tag + " ---->singleReadRFID(Handler handler)--->true");
			codeString = null;
			return true;
		} else {
			System.out.println(tag + "-->codeString=" + codeString);
...
//			msg.what = ScanWriteActivity.readRFID;
			...
			System.out.println(tag + " ---->singleReadRFID(Handler handler)--->false");
			codeString = null;
			return false;
		}
	}
	...
	public void writeRFID(String codeString) {
		...
		System.out.println(tag + "--->writeRFID(String codeString, Handler handler)");
		Message msg = Message.obtain();
		Bundle bundle = new Bundle();
		for (int i = 0; i < 3; i++) {
		...
		public void run() {
			...
			while (isOff) {
				...
				if ("timeout".equals(codeString)) {
					System.out.println("codeString ----------------------->timeout" + codeString);
					isRepeatTimeOut = true;
					continue;
				}
				isRepeatTimeOut = true;
		...
		public void run() {
			...
			temperaterHandler = new Handler() {
				...
				public void handleMessage(Message msg) {
					...
					// 获取温度监控jni接口
					Log.i("RFIDDevice", "temperaterHandler----->msg.what" + msg.what);
					i = rfidSerialPort.gettemp();
					Log.i("RFIDDevice", "temperaterHandler----->	i = rfidSerialPort.gettemp();" + i);
					// end
					if (handler != null) {
						Message mg = Message.obtain();
						mg.what = msg.what;
			...
			Looper.loop();
			Log.i("ThreadManage", "temperaterThread------------------------->end");
		}
	}
	// -------------->end
}
