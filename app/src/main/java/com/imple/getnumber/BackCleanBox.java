...
public class BackCleanBox implements INotify {
	...
	public void getNumber(String number) {
		if (BackMoneyBoxCountDo.hadfindbox || number == null || !CaseString.reg(number)) {
			return;
		}
		...
		if (map.size() <= 0 && mapcount <= 0) {
			...
			for (int i = 0; i < RecycleCashboxCheckDetailBiz.list.size(); i++) {
				map.put(RecycleCashboxCheckDetailBiz.list.get(i).getNum(), RecycleCashboxCheckDetailBiz.list.get(i));
			}
		}
