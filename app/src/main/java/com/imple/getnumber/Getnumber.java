...
public class Getnumber implements INotify {
	...
	GetBoxDetailListBiz getBoxDetailListBiz() {
		return boxDetailListBiz == null ? new GetBoxDetailListBiz() : boxDetailListBiz;
	}
	...
	public void emptycheck(String str) {
		...
		for (int i = 0; i < list_boxdeatil.size(); i++) {
			Log.i("list_boxdeatil.get(i).getNum()", list_boxdeatil.get(i).getNum());
			Log.i("num", str);
	...
	public void temp() {
		...
		if (map.size() <= 0) {
			for (int i = 0; i < getBoxDetailListBiz().list.size(); i++) {
				...
				map.put(getBoxDetailListBiz().list.get(i).getNum(), getBoxDetailListBiz().list.get(i));
			}
		}
		Log.i("map", map.size() + "");
	}
