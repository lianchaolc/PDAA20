...
public class ClearManager extends Activity implements OnTouchListener {
	...
	GetCashboxNumClearCheckBiz getGetCashboxNumClearCheck() {
		return getCashboxNumClearCheck = getCashboxNumClearCheck == null ? new GetCashboxNumClearCheckBiz()
				: getCashboxNumClearCheck;
	}
		...
		if (MotionEvent.ACTION_UP == even.getAction()) {
			...
			case R.id.addmoney:
				...
				if (addboxcount.getText().equals("0")) {
					...
					managerClass.getGolbalutil().gotoActivity(ClearManager.this, PlanWay.class, bundle_biz,
