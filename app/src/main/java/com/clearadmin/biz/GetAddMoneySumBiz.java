...
//钞箱加钞-加钞操作-扫描钞箱编号
	...
	UpdateAndGetAddMoneySumService getUpdateAndGetAddMoneySum() {
		return updateAndGetAddMoneySum == updateAndGetAddMoneySum ? new UpdateAndGetAddMoneySumService()
				: updateAndGetAddMoneySum;
	}
	 ...
	 * @param corpId     机构编号
	 */
	public void getupdateAndGetAddMoneySum(String planNum, String cashboxNum, String userId1, String userId2,
			String corpId) {
	...
	class Run implements Runnable {
		...
		public void run() {
			try {
				box = getUpdateAndGetAddMoneySum().updateAndGetAddMoneySum(planNum, cashboxNum, userId1, userId2,
						corpId);
				if (box != null) {
					m.what = 1; // 有数据
				} else {
