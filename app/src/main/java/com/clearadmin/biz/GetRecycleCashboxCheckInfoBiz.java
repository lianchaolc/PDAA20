...
public class GetRecycleCashboxCheckInfoBiz {
	...
	GetRecycleCashboxCheckInfoService getRecycleCashboxCheckInfo() {
		return RecycleCashboxCheckInfo = RecycleCashboxCheckInfo == null ? new GetRecycleCashboxCheckInfoService()
				: RecycleCashboxCheckInfo;
	}
	 ...
	 * @param corpId     机构ID
	 */
	public void getrecycleCashboxCheckInfo(String orderNum, String cashboxNum, String userId1, String userId2,
			String corpId) {
	...
	class Run implements Runnable {
		...
		public void run() {
			try {
				box = getRecycleCashboxCheckInfo().updateAndGetRecycleCashboxCheckInfo(orderNum, cashboxNum, userId1,
						userId2, corpId);
				if (box != null) {
					m.what = 1;
				} else {
