...
public class LookStorageService {
	 ...
	 * 根据查看员的id获取查看任务
	 * 
	 * @param codeOne 查库员1的id
	 * @param codeTwo 查库员2的id
	 * @return
	 */
	public static List<LookStorage> getTask(String codeOne, String codeTwo) throws Exception {
		...
		WebParameter<?>[] param = { new WebParameter<String>("arg0", codeOne),
		...
		if (code.equals("00")) {
			result = new Gson().fromJson(params, new TypeToken<List<LookStorage>>() {
			}.getType());
		} else {
			return null;
		}
	...
	public static List<LookStorageDetail> getTaskDetail(String taskId) throws Exception {
		...
		WebParameter<?>[] param = { new WebParameter<String>("arg0", taskId) };
		SoapObject soap = WebService.getSoapObject(mothedName, param);
		String code = soap.getProperty("code").toString();
		String params = soap.getProperty("params").toString();
		if (code.equals("00")) {
			result = new Gson().fromJson(params, new TypeToken<List<LookStorageDetail>>() {
			}.getType());
			// 初始化清点状态为失败
	...
	public static boolean commitTask(List<LookStorageDetail> detail) throws Exception {
		...
		WebParameter<?>[] param = { new WebParameter<String>("arg0", json) };
		SoapObject soap = WebService.getSoapObject(mothedName, param);
		String code = soap.getProperty("code").toString();
