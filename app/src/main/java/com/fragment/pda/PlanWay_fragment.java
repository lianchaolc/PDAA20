	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		clickReplaceback = new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				// 参数，机构ID
				getEmptyRecycleCashboxInList().getemptyback(GApplication.user.getOrganizationId());
		...
		getEmptyRecycleCashboxInList().hand_backbox = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				case -1:
					notdata.setVisibility(View.GONE);
					managerClass.getAbnormal().timeout(getActivity(), "连接异常，重新连接？", clickReplaceback);
					break;
				case -4:
					notdata.setVisibility(View.GONE);
					managerClass.getAbnormal().timeout(getActivity(), "连接超时，重新连接？", clickReplaceback);
					break;
				case 0:
					notdata.setVisibility(View.VISIBLE);
					break;
	...
	class WayAd extends BaseAdapter {
		...
		public View getView(int arg0, View view, ViewGroup arg2) {
			...
			if (v == null) {
				...
				v = GolbalView.getLF(getActivity()).inflate(R.layout.c_way_item, null);
				holder.planNum = (TextView) v.findViewById(R.id.plannumber);
				holder.planContent = (TextView) v.findViewById(R.id.plancontent);
			...
			v.setOnTouchListener(new OnTouchListener() {
				...
				public boolean onTouch(View view, MotionEvent even) {
					...
					if (MotionEvent.ACTION_UP == even.getAction()) {
						...
						bundle_listitem.putString("number", holder.planNum.getText().toString());
						bundle_listitem.putString("way", holder.planContent.getText().toString());
						bundle_listitem.putString("businName", busin);
						bundle_listitem.putString("isfirst", holder.isfirst.getText().toString());
