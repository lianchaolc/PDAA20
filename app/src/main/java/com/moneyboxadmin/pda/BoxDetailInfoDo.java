...
public class BoxDetailInfoDo extends Activity implements OnTouchListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// 判断是何种操作
		if ("空钞箱出库".equals(busin) || "ATM加钞出库".equals(busin) || "未清回收钞箱出库".equals(busin)) {
			box_do.setText("出库操作");

		} else if ("钞箱装钞入库".equals(busin) || "回收钞箱入库".equals(busin) || "已清回收钞箱入库".equals(busin)
				|| "新增钞箱入库".equals(busin)) {
			box_do.setText("入库操作");
		...
		// 判断加载那个fragment
		if ("空钞箱出库".equals(busin) || "钞箱加钞".equals(busin) || "ATM加钞出库".equals(busin) || "钞箱装钞入库".equals(busin)) {
			if (emptyf == null) {
				emptyf = new EmptyBoxOut_fragment();
			}
			managerClass.getGolbalView().replaceFragment(this, R.id.fragment_address, emptyf, bundleBussin);
		} else if ("回收钞箱入库".equals(busin)) {
			if (backf == null) {
				backf = new BackMoneyBoxIn_fragment();
			}
			managerClass.getGolbalView().replaceFragment(this, R.id.fragment_address, backf, bundleBussin);
			...
			managerClass.getGolbalView().replaceFragment(this, R.id.fragment_address, clearbackf, bundleBussin);

		} else if ("回收钞箱清点".equals(busin)) { // 界面特殊，这次跳到别的activity
			this.finish();
			managerClass.getGolbalutil().gotoActivity(BoxDetailInfoDo.this, BackMoneyBoxCount.class, bundleBussin,
		 ...
		 */
		getBoxDetailList().hand_detail = new Handler() {
			@Override
			public void handleMessage(Message msg) {
				// TODO Auto-generated method stub
				super.handleMessage(msg);
				String checkTask = (String) msg.getData().get("checkTask");
				switch (msg.what) {
				case -2:
					managerClass.getGolbalView().toastShow(BoxDetailInfoDo.this, "程序出现错误");
					// box_detail.setEnabled(false);
					// box_detail.setBackgroundColor(R.color.gray_background);
					box_detail.setBackgroundResource(R.drawable.gray_btn_bg);
					break;
				case -1:
					managerClass.getGolbalView().toastShow(BoxDetailInfoDo.this, "请求数据异常");
					// box_detail.setEnabled(false);
					box_detail.setBackgroundResource(R.drawable.gray_btn_bg);
					// box_detail.setBackgroundColor(R.color.gray_background);
					break;
				case 12:
					managerClass.getGolbalView().toastShow(BoxDetailInfoDo.this, "该任务中还有未进行“钞箱加钞”操作的钞箱，无法继续进行！！！");
					// box_detail.setEnabled(false);
					box_detail.setBackgroundResource(R.drawable.gray_btn_bg);
					// box_detail.setBackgroundColor(R.color.gray_background);
					break;
				case 11:
					managerClass.getGolbalView().toastShow(BoxDetailInfoDo.this, "该任务已进行“钞箱加钞”操作！");
					box_detail.setEnabled(true);
	...
	public boolean onTouch(View view, MotionEvent even) {
		...
		if (MotionEvent.ACTION_UP == even.getAction()) {
			...
			case R.id.money_box_detail:
				box_detail.setBackgroundResource(R.drawable.buttom_selector_bg);
				if (isfirst > 0) {
					managerClass.getSureCancel().makeSuerCancel(BoxDetailInfoDo.this, "首次回收钞箱入库不用获取明细",
					...
					managerClass.getGolbalutil().gotoActivity(BoxDetailInfoDo.this, MoneyBoxDetial.class, bundleBussin,
							managerClass.getGolbalutil().ismover);
				}
				break;
			// 各种操作，出库
			case R.id.box_do:
				// //出库前清空前一次的数据
				if (Getnumber.list_boxdeatil.size() > 0 || Getnumber.map.size() > 0) {
					try {
				...
				// 如果是首次回收钞箱入库
				if (isfirst > 0) {
					managerClass.getGolbalutil().gotoActivity(BoxDetailInfoDo.this, BoxDoDetail.class, bundleBussin,
							managerClass.getGolbalutil().ismover);
				} else {

					if ("钞箱加钞".equals(busin)) {
						if (getCashboxAddMoneyDetail().list == null || getCashboxAddMoneyDetail().list.size() <= 0) {
							GolbalView.toastShow(BoxDetailInfoDo.this, "请先获取钞箱明细，再进行出入库。");
						} else {

							managerClass.getGolbalutil().gotoActivity(BoxDetailInfoDo.this, ClearAddMoneyOutDo.class,
									bundleBussin, managerClass.getGolbalutil().ismover);
						}

					} else {
						if (getBoxDetailList().list == null || getBoxDetailList().list.size() <= 0) {
							GolbalView.toastShow(BoxDetailInfoDo.this, "请先获取钞箱明细，再进行出入库。");
						} else {

							managerClass.getGolbalutil().gotoActivity(BoxDetailInfoDo.this, BoxDoDetail.class,
									bundleBussin, managerClass.getGolbalutil().ismover);
						}
					}
	...
	protected void onStart() {
		// TODO Auto-generated method stub
		super.onStart();
		if ("钞箱装钞入库".equals(busin)) {
			// 判断清分员是否已经进行钞箱加钞，如果没有，则不允许进行“钞箱装钞入库”
			box_detail.setEnabled(false);
			box_detail.setBackgroundResource(R.drawable.gray_btn_bg);
			getBoxDetailList().checkTaskState(planNum);
		}
