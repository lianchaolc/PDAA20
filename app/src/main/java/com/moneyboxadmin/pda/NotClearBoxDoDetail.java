 ...
 * 
 */
public class NotClearBoxDoDetail extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
			...
			public void onClick(View arg0) {
				...
				if (bussin.equals("空钞箱出库")) {
					...
					if (Getnumber.list_boxdeatil.size() == GetBoxDetailListBiz.boxCount) {
						Map<String, Object> resMap = ch.check();
						int rightNum = (Integer) resMap.get("code");

						// if (rightNum != GetBoxDetailListBiz.boxCount ) {//扫描有问题
						if (rightNum != 0) {// 扫描有问题
							// ch.remove();
							ad.notifyDataSetChanged();
							/*
							 * GolbalView.toastShow(BoxDoDetail.this, resMap.get("msg").toString());
							 */
							GolbalView.dialogShow(NotClearBoxDoDetail.this, resMap.get("msg").toString());
						} else {// 扫描成功
							out.setEnabled(true);
						...
						GolbalView.toastShow(NotClearBoxDoDetail.this,
								"当前钞箱的数量与出库的数量不相等(" + GetBoxDetailListBiz.boxCount + ")");
					}

					// 其他业务钞箱检测（ATM加钞出库，钞箱装钞入库，回收钞箱入库钞箱,已清回收钞箱）
				} else {
					if (Getnumber.list_boxdeatil.size() == GetBoxDetailListBiz.boxCount) {
						String cboxnums = ch.checkBox(Getnumber.list_boxdeatil);
						if (cboxnums.equals("00")) {
							...
							GolbalView.toastShow(NotClearBoxDoDetail.this, cboxnums + "为无效钞箱");
						}
					} else {
						GolbalView.toastShow(NotClearBoxDoDetail.this,
								"当前钞箱的数量与出库的数量不相等(" + GetBoxDetailListBiz.boxCount + ")");
		...
		clickreplace = new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				/*
				 * getMoneyBoxOutDo().getemptyMoneyBoxoutdo(bussin, GetBoxDetailListBiz.list,
				 * planNum, BankDoublePersonLogin.userid1, BankDoublePersonLogin.userid2,
				...
				managerClass.getRuning().runding(NotClearBoxDoDetail.this, "正在出库...");
		...
		getMoneyBoxOutDo().hand_out = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				switch (msg.what) {
				case 1:
					managerClass.getResultmsg().resultmsg(NotClearBoxDoDetail.this, bussin + "成功");
					// 2秒后跳转
					OutDo();
					break;

				case -1:
					managerClass.getAbnormal().timeout(NotClearBoxDoDetail.this, "连接超时，要重试吗？", clickreplace);
					break;

				case 0:
					managerClass.getSureCancel().makeSuerCancel(NotClearBoxDoDetail.this, "操作失败",
							new OnClickListener() {
								@Override
								public void onClick(View arg0) {
	...
	public void isEnough() {
		...
		if (bussin.equals("空钞箱出库")) {
			...
			if (getGetnumber().list_boxdeatil.size() >= GetBoxDetailListBiz.boxCount) {
				Log.i("GetBoxDetailListBiz.boxCount", GetBoxDetailListBiz.boxCount + "");
				Log.i("getGetnumber().list_boxdeatil.size()", getGetnumber().list_boxdeatil.size() + "");
				// 暂停扫描
				managerClass.getRfid().stop_a20();
			}
			...
			// 扫到足够的箱子的时候就关闭扫描
			if (getGetnumber().list_boxdeatil.size() >= GetBoxDetailListBiz.list.size()) {
				// 暂停扫描
				managerClass.getRfid().stop_a20();
				out.setEnabled(true);
				out.setBackgroundResource(R.drawable.buttom_selector_bg);
		...
		public boolean onTouch(View view, MotionEvent even) {
			...
			if (MotionEvent.ACTION_DOWN == even.getAction()) {
				...
				case R.id.outdo_back:
					outdo_back.setBackgroundResource(R.drawable.back_cirle_press);
					break;
				}
			}
			...
			if (MotionEvent.ACTION_UP == even.getAction()) {
				...
				case R.id.out:
					...
					if (bussin.equals("ATM加钞出库")) {
						...
						// 在行式
						managerClass.getGolbalutil().gotoActivity(NotClearBoxDoDetail.this, SupercargoJoin.class,
								bundleBussin, 0);

					} else {
						// 提示正在出库
						managerClass.getRuning().runding(NotClearBoxDoDetail.this, "正在执行操作...");
				...
				case R.id.outdo_back:
					...
					managerClass.getSureCancel().makeSuerCancel(NotClearBoxDoDetail.this, "确定要退出当前页面吗？",
							new OnClickListener() {
								@Override
								public void onClick(View arg0) {
									managerClass.getSureCancel().remove();
									NotClearBoxDoDetail.this.finish();
								}
							}, false);
					break;
	...
	class Ad extends BaseAdapter {
		...
		public View getView(int arg0, View view, ViewGroup arg2) {
			...
			if (v == null) {
				...
				v = GolbalView.getLF(NotClearBoxDoDetail.this).inflate(R.layout.boxinformation_item, null);
				v.setTag(holder);
			...
			// holder.number.setText(box.getAtmType());
				...
				public boolean onTouch(View view, MotionEvent even) {
					...
					if (MotionEvent.ACTION_UP == even.getAction()) {
						...
						for (int i = 0; i < Getnumber.list_boxdeatil.size(); i++) {
							if (holder.number.getText().equals(Getnumber.list_boxdeatil.get(i).getNum())) {
								// 删除掉一条数据后，开始扫描
								managerClass.getRfid().start_a20();

								// 移除一条数据
								Getnumber.list_boxdeatil.remove(i);
								Log.i("Getnumber.list_boxdeatil", Getnumber.list_boxdeatil.size() + "");
								boxcount.setText(getGetnumber().list_boxdeatil.size() + "");
								ad.notifyDataSetChanged();
								out.setEnabled(false);
								out.setBackgroundResource(R.drawable.button_gray);
			...
			// 品牌触摸事件
			holder.number.setOnTouchListener(new OnTouchListener() {
				@Override
				...
				public boolean onTouch(View vi, MotionEvent even) {
					...
					if (MotionEvent.ACTION_UP == even.getAction()) {
						...
						if (GolbalUtil.ismover <= 5 && holder.brand.getText().equals("点击获取品牌")) {
							...
							// 通过编号获取品牌
							getBrandNameByCboxNumBiz().getBrnadNamebyNum(holder.number.getText().toString(), "空钞箱出库");
						}
						GolbalUtil.ismover = 0;
					}
					// 手指移动的时候
		...
		t2.schedule(new TimerTask() {
			...
			public void run() {
				...
				if ("空钞箱出库".equals(bussin) || "未清回收钞箱出库".equals(bussin)) {
					...
					managerClass.getGolbalutil().gotoActivity(NotClearBoxDoDetail.this, BankDoublePersonLogin.class,
					...
					// 不用交接，进入页面后直接完成
					managerClass.getGolbalutil().gotoActivity(NotClearBoxDoDetail.this, BoxAddStop.class, bundleBussin,
							managerClass.getGolbalutil().ismover);
				} else if ("钞箱装钞入库".equals(bussin) || "回收钞箱入库".equals(bussin) || "已清回收钞箱入库".equals(bussin)) {
					managerClass.getGolbalutil().gotoActivity(NotClearBoxDoDetail.this, JoinResult.class, bundleBussin,
							managerClass.getGolbalutil().ismover);
					// 弹栈
