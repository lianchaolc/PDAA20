...
import com.example.app.activity.YayunCheckFingerActivity;
...
//押运员交接
public class SupercargoJoin extends BaseFingerActivity implements OnTouchListener {
    public boolean istrueoperncodecheck = false;//  默认不开启三次指纹验证
    ...
    ATMOutAndJoinBiz getaTMOutAndJoinBiz() {
        return aTMOutAndJoinBiz = aTMOutAndJoinBiz == null ? new ATMOutAndJoinBiz() : aTMOutAndJoinBiz;
    }
...
//		Log.i("plan", plan);
		...
		/*
         * ATM加钞出库交接 Revised by zhangXueWei
		 */
            ...
            public void handleMessage(Message msg) {
                ...
                switch (msg.what) {
                    case 1:// 验证通过
                        ...
                        managerClass.getSureCancel().makeSuerCancel2(SupercargoJoin.this, "押运员：" + name,
...
//						budnle.putString("FLAG", "chukuyanzhengzhanghao");
//						ShareUtil.finger_gather = null;
//						intent.putExtras(budnle);
//						Skip.skip(SupercargoJoin.this, YayunCheckFingerActivity.class, budnle, 0);

                            Bundle bundle2 = new Bundle();
                            Intent intent1 = new Intent(SupercargoJoin.this, SupercarYayyCheckFActivity.class);
                            bundle2.putString("FLAG", "chukuyanzhengzhanghao");
                            ShareUtil.finger_gather = null;
            ...
            public void handleMessage(Message msg) {
                ...
                switch (msg.what) {
                    case 1:
                        ...
                        managerClass.getSureCancel().makeSuerCancel(SupercargoJoin.this, "交接成功",
                                new View.OnClickListener() {
                                    @Override
                                    public void onClick(View arg0) {
                                        managerClass.getSureCancel().remove();// revised by zhangxuewei
                                        managerClass.getGolbalutil().gotoActivity(SupercargoJoin.this, JoinResult.class,
                                                bundleIntent, 0);
                                        // 重置交接状态
                                        isscuess = false;
                                        // 关闭当前页面
                    ...
                    case -1:
                        ...
                        managerClass.getAbnormal().timeout(SupercargoJoin.this, "出库交接异常,重试请确定！",
                                new View.OnClickListener() {
                                    @Override
                                    public void onClick(View arg0) {
                                        ...
                                        Log.i("我又来交接了", "123");
                                        // 拼参数
                                        String cbinfo = getaTMOutAndJoinBiz().getCbInfo(plan, Getnumber.list_boxdeatil,
                                        ...
                                        getaTMOutAndJoinBiz().atmOutAndJoin(cbinfo,
                                                GApplication.user.getOrganizationId(),
                                                FixationValue.supercargo + "", ShareUtil.ivalBack, "", "", GApplication.user.getYonghuZhanghao());
                                    }
                                });
                    ...
                    case 0:
                        ...
                        managerClass.getSureCancel().makeSuerCancel(SupercargoJoin.this, "钞箱交接失败",
                                new View.OnClickListener() {
                                    @Override
                                    public void onClick(View arg0) {
                                        managerClass.getSureCancel().remove();
                    ...
                    case -4:
                        ...
                        managerClass.getAbnormal().timeout(SupercargoJoin.this, "出库交接超时,重试请确定！",
                                new View.OnClickListener() {
                                    @Override
                                    public void onClick(View arg0) {
                                        managerClass.getAbnormal().remove();
            ...
            public void handleMessage(Message msg) {
                ...
                switch (msg.what) {
                    ...
                    case -1:
                        ...
                        managerClass.getAbnormal().timeout(SupercargoJoin.this, "连接异常，重试请确定！", new View.OnClickListener() {
                            @Override
                            public void onClick(View arg0) {
                                supertext.setText("正在交接...");
                                managerClass.getRuning().runding(SupercargoJoin.this, "正在交接...");
                                getSaveAuthLogBiz().getSaveAuthLog(plan, GetBoxDetailListBiz.list,
                                        GApplication.getApplication().user.getOrganizationId(),
                                        FixationValue.supercargo + "", ShareUtil.ivalBack, "04", "1", "1");
                    ...
                    case -6:
                        supertext.setText("交接超时..");
                        managerClass.getAbnormal().timeout(SupercargoJoin.this, "连接超时，重试请确定！", new View.OnClickListener() {
                            @Override
                            public void onClick(View arg0) {
                                supertext.setText("正在交接...");
                                managerClass.getRuning().runding(SupercargoJoin.this, "正在交接...");
    ...
    public void towseconds() {
        timer.schedule(new TimerTask() {
            ...
            public void run() {
                ...
                managerClass.getGolbalutil().gotoActivity(SupercargoJoin.this, JoinResult.class, bundleIntent, 0);
                // 关闭当前页面
                SupercargoJoin.this.finish();
            }
        }, 2000);
    ...
    public void join(String biz) {
        if (biz.equals("ATM加钞出库")) {
            ...
            String cbinfo = getaTMOutAndJoinBiz().getCbInfo(plan, Getnumber.list_boxdeatil,
                    BankDoublePersonLogin.userid1, BankDoublePersonLogin.userid2,
                    GApplication.user.getOrganizationId());
            Log.i("ATM加钞出库"+"SupercargoJoin", biz + "走老方法s");
            getaTMOutAndJoinBiz().atmOutAndJoin(cbinfo, GApplication.user.getOrganizationId(),
            ...
            if ("在行式".equals(OrderWork.type)) {
                ...
                getSaveAuthLogBiz().getSaveAuthLog(plan, CashboxNumByCorpIdBiz.list,
                        GApplication.user.getOrganizationId(), FixationValue.supercargo + "",
                        ShareUtil.ivalBack, type, "1", "1");
            } else {
                getSaveAuthLogBiz().getSaveAuthLog(plan, CashboxNumByCorpIdBiz.list,
                        GApplication.user.getOrganizationId(), FixationValue.supercargo + "",
                        ShareUtil.ivalBack, "C1", "1", "1");
            }
        }
    }
    ...
    private class IsKongxian implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                String code = new SaveAuthLogService().checkEscort1State(user.getLoginUserId());
    ...
    private class IsKongxianBypwyyy implements Runnable {
        private SystemUser user;

        public IsKongxianBypwyyy(SystemUser user) {
            this.user = user;
        }

        @Override
        public void run() {
            Message msg = kongxianHandelrBypwyyy.obtainMessage();
            try {
                System.out.println("钓调用空闲查询——参数userId:" + user.getLoginUserId());
                String code = new SaveAuthLogService().checkEscort1State(user.getLoginUserId());
                System.out.println("钓调用空闲查询——结果code:" + code);
                ...
                kongxianHandelrBypwyyy.sendMessage(msg);
            }
        }
    }
     ...
     */
    @SuppressLint("HandlerLeak")
    private Handler kongxianHandelrBypwyyy = new Handler() {
        public void handleMessage(Message msg) {
            SystemUser user = (SystemUser) msg.obj;
            switch (msg.what) {
                case 1: // 空闲(可用)
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 6) {
            if (null != data) {
                final SystemUser result_user = new SystemUser();
                String result1 = data.getStringExtra("userId");
                String username = data.getStringExtra("username");
                System.out.println("钓调用空闲查询——参数userId:" + result1);
                System.out.println("钓调用空闲查询——参数userId:" + username);
                Toast.makeText(SupercargoJoin.this, "yazheng " + result1 + username, Toast.LENGTH_SHORT).show();
                result_user.setLoginUserId(result1);
                result_user.setLoginUserName(username);
                if (busin.equals("ATM加钞出库")) {
                    supername.setText("" + username);
                    Thread thread = new Thread(new IsKongxianBypwyyy(result_user));
                    thread.start();
                } else {// 回收空钞箱交接
                    supertext.setText("开始交接...");
//					supername.setText(name);
...
//	arg8=押运员
     ...
     * @param biz
     */
    public void joinByPW(String biz) {
        if (biz.equals("ATM加钞出库")) {
            supertext.setText("正在出库交接...");
            Log.i("ATM加钞出库", biz + "");
            String cbinfo = getaTMOutAndJoinBiz().getCbInfo(plan, Getnumber.list_boxdeatil,
            ...
            byte[] cValue=null;
            getaTMOutAndJoinBiz().atmOutAndJoin(cbinfo, GApplication.user.getOrganizationId(),
                    FixationValue.supercargo + "", cValue, "", "", GApplication.user.getYonghuZhanghao());
        } else if (biz.equals("网点回收钞箱交接")) {
            // 网点人员交接
            Log.i("网点回收钞箱交接", biz + "");
            supertext.setText("正在交接...");
            ...
            if ("在行式".equals(OrderWork.type)) {
                // 在行式交接 type传"04" 离行式交接传"C1"
                getSaveAuthLogBiz().getSaveAuthLogbyPW(plan, CashboxNumByCorpIdBiz.list,
                        GApplication.user.getOrganizationId(), FixationValue.supercargo + "",
                        cValue, type, "3", "1", GApplication.user.getYonghuZhanghao());
            } else {
                getSaveAuthLogBiz().getSaveAuthLogbyPW(plan, CashboxNumByCorpIdBiz.list,
                        GApplication.user.getOrganizationId(), FixationValue.supercargo + "",
                        cValue, "C1", "3", "1", GApplication.user.getYonghuZhanghao());
            }
        }
    }
}
