	...
	protected void onCreate(Bundle savedInstanceState) {
				...
				if (bussin.equals("空钞箱出库")) {
					...
					if (Getnumber.list_boxdeatil.size() == GetBoxDetailListBiz.boxCount) {
						...
						if (rightNum != 0) {// 扫描有问题
							 ...
							 */
							GolbalView.dialogShow(BoxDoDetail.this, resMap.get("msg").toString());
						} else {// 扫描成功
							out.setEnabled(true);
					...
					if (Getnumber.list_boxdeatil.size() == GetBoxDetailListBiz.boxCount) {
						...
						if (cboxnums.equals("00")) {
							...
							GolbalView.toastShow(BoxDoDetail.this, cboxnums + "为无效钞箱");
						}
					} else {
						GolbalView.toastShow(BoxDoDetail.this,
								"当前钞箱的数量与出库的数量不相等(" + GetBoxDetailListBiz.boxCount + ")");
		...
		clickreplace = new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if (bussin.equals("空钞箱出库")) {
					getMoneyBoxOutDo().getemptyMoneyBoxoutdo(bussin, Getnumber.list_boxdeatil, planNum,
					...
					getMoneyBoxOutDo().getemptyMoneyBoxoutdo(bussin, GetBoxDetailListBiz.list, planNum,
							BankDoublePersonLogin.userid1, BankDoublePersonLogin.userid2,
							GApplication.user.getOrganizationId(), bizNum, BoxDetailInfoDo.isfirst);
				}
				// 移除超时提示
		...
		getMoneyBoxOutDo().hand_out = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				switch (msg.what) {
				case 1:
					managerClass.getResultmsg().resultmsg(BoxDoDetail.this, bussin + "成功");
					// 2秒后跳转
					OutDo();
					break;

				case -1:
					managerClass.getAbnormal().timeout(BoxDoDetail.this, "连接超时，要重试吗？", clickreplace);
					break;

				case 0:
					managerClass.getSureCancel().makeSuerCancel(BoxDoDetail.this, "操作失败", new OnClickListener() {
						@Override
						public void onClick(View arg0) {
			...
			if (MotionEvent.ACTION_UP == even.getAction()) {
				...
				case R.id.out:
					...
					if (bussin.equals("ATM加钞出库")) {
						...
						// 在行式
						managerClass.getGolbalutil().gotoActivity(BoxDoDetail.this, SupercargoJoin.class, bundleBussin,
								0);

					} else {
						// 提示正在出库
						managerClass.getRuning().runding(BoxDoDetail.this, "正在执行操作...");
				...
				case R.id.outdo_back:
					...
					managerClass.getSureCancel().makeSuerCancel(BoxDoDetail.this, "确定要退出当前页面吗？", new OnClickListener() {
						@Override
						public void onClick(View arg0) {
							managerClass.getSureCancel().remove();
	...
	class Ad extends BaseAdapter {
		...
		public View getView(int arg0, View view, ViewGroup arg2) {
			...
			if (v == null) {
				...
				v = GolbalView.getLF(BoxDoDetail.this).inflate(R.layout.boxinformation_item, null);
				v.setTag(holder);
		...
		t2.schedule(new TimerTask() {
			...
			public void run() {
				...
				if ("空钞箱出库".equals(bussin) || "未清回收钞箱出库".equals(bussin)) {
					...
					managerClass.getGolbalutil().gotoActivity(BoxDoDetail.this, BankDoublePersonLogin.class,
					...
					// 不用交接，进入页面后直接完成
					managerClass.getGolbalutil().gotoActivity(BoxDoDetail.this, BoxAddStop.class, bundleBussin,
							managerClass.getGolbalutil().ismover);
				} else if ("钞箱装钞入库".equals(bussin) || "回收钞箱入库".equals(bussin) || "已清回收钞箱入库".equals(bussin)) {
					managerClass.getGolbalutil().gotoActivity(BoxDoDetail.this, JoinResult.class, bundleBussin,
							managerClass.getGolbalutil().ismover);
					// 弹栈
