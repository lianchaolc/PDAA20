...
public class BoxAddStop extends Activity {
	...
	GetBrandNameByCboxNumService getBoxService() {
		return boxService = boxService == null ? new GetBrandNameByCboxNumService() : boxService;
	}
	...
	protected void onCreate(Bundle savedInstanceState) {
		 ...
		 * managerClass.getRuning().remove(); super.handleMessage(msg);
		 * 
		 * switch (msg.what) { case 1: // 信息提示 Log.i("sssssss", "ssssssssss");
		 * managerClass.getGolbalView().toastShow(BoxAddStop.this, "获取成功，开启扫描"); // 打开扫描
		 * getRfid().open_a20();
		 * 
		 * break; case -1: managerClass.getAbnormal().timeout(BoxAddStop.this,
		 * "获取异常！重试请确定", new View.OnClickListener() {
		 * 
		 * @Override public void onClick(View arg0) {
		 * managerClass.getAbnormal().remove(); managerClass.getRuning().runding(
		 * BoxAddStop.this, "正在获取未清钞箱明细"); getBoxDetailListBiz().getBoxDetailList( ,
		 * "未清回收钞箱出库"); } }); break; case 0:
		 * managerClass.getSureCancel().makeSuerCancel( BoxAddStop.this, "获取失败,找不到数据",
		 * new View.OnClickListener() {
		 * 
		 * @Override public void onClick(View arg0) {
		 * managerClass.getSureCancel().remove(); } }, true); break; case -4:
		 * managerClass.getAbnormal().timeout(BoxAddStop.this, "获取超时！重试请确定", new
		...
		if ("停用钞箱出库".equals(busin)) {
			 ...
			 * getBoxDetailListBiz().getBoxDetailList( , "未清回收钞箱出库");
			 */
			managerClass.getGolbalView().toastShow(BoxAddStop.this, "开启扫描功能");
			getRfid().open_a20();
		...
		clickreplace = new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				managerClass.getRuning().runding(BoxAddStop.this, "操作正在执行,请等待...");
			}
		};
		...
		clickreplace2 = new OnClickListener() {
			...
			public void onClick(View arg0) {
				getRfid().close_a20();
				managerClass.getRuning().runding(BoxAddStop.this, "正在核实钞箱信息...");
...
//								bundle_biz, 0);
//					} else {
					managerClass.getResultmsg().resultmsg(BoxAddStop.this, busin + "成功");
					OutDo();
//					}

					break;
				case 0:
					managerClass.getSureCancel().makeSuerCancel(BoxAddStop.this, busin + "失败", new OnClickListener() {
						@Override
						public void onClick(View arg0) {
					...
					break;
				case -1:
					managerClass.getAbnormal().timeout(BoxAddStop.this, "连接异常，要重试吗？", clickreplace);
					break;
				case -4:
					managerClass.getAbnormal().timeout(BoxAddStop.this, "连接超时，要重试吗？", clickreplace);
					break;
				}
			...
			if (MotionEvent.ACTION_UP == even.getAction()) {
				...
				case R.id.noway_out:
					...
					if ("未清回收钞箱出库".equals(busin)) {
						...
						Log.d("boxAddStop" + "", "bundle_biz====" + bundle_biz.toString());
						managerClass.getGolbalutil().gotoActivity(BoxAddStop.this, BankDoublePersonLogin.class,
								bundle_biz, 0);
					} else {
						managerClass.getRuning().runding(BoxAddStop.this, "操作正在执行...");
						getMoneyBoxOutDo().getemptyMoneyBoxoutdo(busin, StopNewClearBox.list, planNum,
								BankDoublePersonLogin.userid1, BankDoublePersonLogin.userid2,
								GApplication.user.getOrganizationId(), bizNum, BoxDetailInfoDo.isfirst);
					}
				...
				case R.id.outberforcheck:
					...
					// 检测是否所有钞箱都已经获取到了品牌
					/**
					 * modify by wangmeng 2017-07-20
					 */
					if (StopNewClearBox.list != null && GetBoxDetailListBiz.list != null
							&& StopNewClearBox.list.size() == GetBoxDetailListBiz.list.size()) {
						String cboxnums = ch.checkBox(StopNewClearBox.list);
						if (cboxnums.equals("00")) {
							/** end */
						...
						} else if (cboxnums.length() > 2) {
							GolbalView.toastShow(BoxAddStop.this, cboxnums + "为无效钞箱");
						}
					} else {
						if (GetBoxDetailListBiz.list != null)
							GolbalView.toastShow(BoxAddStop.this, "钞箱数量不正确，总数为：" + GetBoxDetailListBiz.list.size());
						else
							GolbalView.toastShow(BoxAddStop.this, "钞箱数量不正确，总数为：空");
					}
	...
	class Ad extends BaseAdapter {
		...
		public View getView(int arg0, View view, ViewGroup arg2) {
			...
			if (v == null) {
				v = GolbalView.getLF(BoxAddStop.this).inflate(R.layout.boxinformation_item, null);
				holder = new ViewHolder();
				holder.brand = (TextView) v.findViewById(R.id.brand);
				holder.Num = (TextView) v.findViewById(R.id.box_num);
				holder.delText = (TextView) v.findViewById(R.id.newstop_item_del);
				holder.cancelText = (TextView) v.findViewById(R.id.newstop_item_cancel);
				holder.llayout = (LinearLayout) v.findViewById(R.id.lboxdodetail_item);
				holder.menu = (LinearLayout) v.findViewById(R.id.newstop_item_menu);
			...
			holder.llayout.setOnTouchListener(new OnTouchListener() {
				...
				public boolean onTouch(View view, MotionEvent even) {
					// 按下的时候
					if (MotionEvent.ACTION_DOWN == even.getAction()) {
						holder.llayout.setBackgroundResource(R.color.gray_msg_bg);
					}
					// 手指松开的时候
					if (MotionEvent.ACTION_UP == even.getAction()) {
						holder.llayout.setBackgroundResource(R.color.transparency);
					}
			...
			holder.brand.setOnTouchListener(new OnTouchListener() {
				@Override
				public boolean onTouch(View vi, MotionEvent even) {
					...
					if (MotionEvent.ACTION_UP == even.getAction()) {
						...
						if (GolbalUtil.ismover <= 5 && holder.brand.getText().equals("点击获取品牌")) {
							holder.brand.setText("正在获取...");
							// 通过编号获取品牌
							getBrandNameByCboxNumBiz().getBrnadNamebyNum(holder.Num.getText().toString(), "");
						}
						GolbalUtil.ismover = 0;
					}
					// 手指移动的时候
	...
	public void OutDo() {
		...
		timer1.schedule(new TimerTask() {
			...
			public void run() {
				time1500();
				managerClass.getGolbalutil().gotoActivity(BoxAddStop.this, MoneyBoxManager.class, bundle_biz,
						GolbalUtil.ismover);
				StopNewClearBox.liststr.clear();
			}
		}, 2000);
	...
	private class boxList implements Runnable {
		...
		public void run() {
			// TODO Auto-generated method stub
			try {
				List<BoxDetail> list = getBoxService().getBoxDetailInATM(boxNum);
				msg.what = 1;
				msg.obj = list;
	...
	private Handler boxListHandelr = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 3:
				managerClass.getAbnormal().timeout(BoxAddStop.this, "连接超时，要重试吗？", clickreplace2);
				break;
			case -1:
				managerClass.getAbnormal().timeout(BoxAddStop.this, "连接异常，要重试吗？", clickreplace2);
				break;
			}
		}
