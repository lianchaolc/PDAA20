...
public class BrandNameByCboxNumBiz {
	...
	class Run implements Runnable {
		...
		public void run() {
			try {
				String params = getGetBrandNameByCboxNumService().getBrandNameByCboxNum(CboxNum);
	...
	private void addBrand(String CboxNum) {
		for (int i = 0; i < Getnumber.list_boxdeatil.size(); i++) {
			...
			if (num.equals(CboxNum)) {
				...
				Getnumber.list_boxdeatil.get(i).setBoxState(paramArr[1]);
				Getnumber.list_boxdeatil.get(i).setAtmType(paramArr[2]);
				break;
			}
		}
	}
	...
	private void addBrandOther(String CboxNum) {
		for (int i = 0; i < StopNewClearBox.list.size(); i++) {
			...
			if (num.equals(CboxNum)) {
				...
				StopNewClearBox.list.get(i).setBoxState(paramArr[1]);
				StopNewClearBox.list.get(i).setAtmType(paramArr[2]);
				break;
			}
		}
	}
