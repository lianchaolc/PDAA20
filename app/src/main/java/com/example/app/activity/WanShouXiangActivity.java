...
public class WanShouXiangActivity extends Activity implements OnClickListener {
	...
	private TextView oxianlu, kuanxiangNum, ws_count, title_jjName;// 页面标题名称
    ...
    private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 1:
				 ...
				 */
				if (flag == 1 && GApplication.zaolist.size() != GApplication.linshilist.size()
						&& GApplication.jiaojiestate == 3) {
					shouxiang.setBackgroundResource(R.drawable.gray_btn_bg);
					shouxiang.setEnabled(false);
					// adapter.notifyDataSetChanged();
				}

				if (flag == 0 && GApplication.jiaojiestate == 3
						&& GApplication.zyzaolist.size() == GApplication.linshilist.size()) {
					shouxiang.setBackgroundResource(R.drawable.buttom_selector_bg);
					shouxiang.setEnabled(true);
					if (cscount1 == 0) {
						cscount1++;
					...
					adapter.notifyDataSetChanged();
				} else if (flag == 1 || GApplication.zaolist.size() != GApplication.linshilist.size()
						&& GApplication.jiaojiestate == 3) {
					shouxiang.setBackgroundResource(R.drawable.gray_btn_bg);
					shouxiang.setEnabled(false);
					adapter.notifyDataSetChanged();
	...
	class ShowListView extends BaseAdapter {
		...
		public View getView(final int arg0, View arg1, ViewGroup arg2) {
			...
			if (v == null) {
				...
				holder.llayout = (LinearLayout) v.findViewById(R.id.lboxdodetail_itemcopy);
				v.setTag(holder);
			} else {
			...
			holder.llayout.setOnTouchListener(new OnTouchListener() {
				...
				public boolean onTouch(View view, MotionEvent even) {
					...
					if (MotionEvent.ACTION_UP == even.getAction()) {
						...
						manager.getSureCancel().makeSuerCancel(WanShouXiangActivity.this,
								"确认删除" + ((BoxDetail) getItem(arg0)).getNum() + "?", new OnClickListener() {
									...
									public void onClick(View view) {
										...
										for (int i = 0; i < GApplication.linshilist.size(); i++) {
											...
											System.out.println("box" + box.getNum());
											if (box.getNum().equals(num)) {
												System.out.println("删掉无用钞箱" + GApplication.linshilist.get(i).getNum());
										...
										System.out.println("zyzaolist" + GApplication.zyzaolist.size());
										System.out.println("zaolist" + GApplication.zaolist.size());
										System.out.println(GApplication.linshilist.size());
										if (aa == 0
												&& GApplication.zyzaolist.size() == GApplication.linshilist.size()) {
