...
public class LookStorageTaskDetailActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		adapter = new SimpleAdapter(this, this.orderedView, R.layout.storage_detail_item,
				new String[] { "boxType", "boxNum", "boxState" },
				new int[] { R.id.tv_box_type, R.id.tv_box_num, R.id.tv_box_state });
	...
	public void commitTask(View v) {
		new Thread() {
			public void run() {
				try {
					LookStorageService
							.commitTask(GApplication.taskDetails.get(LookStorageTaskDetailActivity.this.taskId));
	...
	/**
	 * 使orderedView从orderedModel中加载数据 把箱子类型、清点状态有标识码转化为对应的汉字名称
	 * 
	 * @param orderedModel
	 */
	...
	private String parseBoxType(String boxType) {
		return boxType.equals(LookStorageDetail.CX) ? "钞箱"
				: boxType.equals(LookStorageDetail.KX) ? "款箱" : boxType.equals(LookStorageDetail.ZZX) ? "周转箱" : "类别有误";
	}
