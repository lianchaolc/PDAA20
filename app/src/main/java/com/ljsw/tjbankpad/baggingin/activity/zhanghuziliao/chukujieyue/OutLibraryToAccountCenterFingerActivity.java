...
public class OutLibraryToAccountCenterFingerActivity extends BaseFingerActivity{
	...
	private String covun = ""; // 上个页面账户中心借阅查看任务的任务号
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		intent = getIntent();
		covun = intent.getStringExtra("cvoun");
	...
	class CheckFingerThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				System.out.println("GApplication.user.getLoginUserId():" + "货位" + 26 + "==="
...
//				GApplication.user.getLoginUserId()=27+"";
				...
				if (result_user != null) {// 验证成功
					...
					userId = GApplication.use.getUserzhanghu();/// 登录账户
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (data != null) {
			...
			if (isOk.equals("success")) {
				fname.setText(o_Application.yayunyuan.getLoginUserName());// 显示登录时传过来的人员名字 lc20190402
