	...
	public void getNumber(String number) {
		...
		if (!newDate.equals(nowDate)) {
			...
			for (int i = 0; i < S_application.getApplication().leftlist.size(); i++) {
				if (S_application.getApplication().leftlist.get(i).equals(S_application.getApplication().bianhao)) {
					aa = i;
					break;
				}
			}
			...
			if (aa != -1) {
				...
				for (int i = 0; i < S_application.getApplication().rightlist.size(); i++) {
					if (S_application.getApplication().rightlist.get(i)
							.equals(S_application.getApplication().bianhao)) {
