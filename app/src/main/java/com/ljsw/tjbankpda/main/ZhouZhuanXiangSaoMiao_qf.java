 ...
 * @author FUHAIQING
 */
public class ZhouZhuanXiangSaoMiao_qf extends FragmentActivity implements OnClickListener {
...
//				}else{
	...
	private Handler timeoutHandle = new Handler() {// 连接超时handler
		public void handleMessage(Message msg) {
			...
			saomiaoFlag = true;
			if (msg.what == 0) {
				manager.getAbnormal().timeout(ZhouZhuanXiangSaoMiao_qf.this, "数据连接超时", new OnClickListener() {
					@Override
					public void onClick(View arg0) {
						manager.getAbnormal().remove();
						manager.getRuning().runding(ZhouZhuanXiangSaoMiao_qf.this, "数据加载中...");
			...
			if (msg.what == 1) {
				manager.getAbnormal().timeout(ZhouZhuanXiangSaoMiao_qf.this, "网络连接失败", new OnClickListener() {
					@Override
					public void onClick(View arg0) {
						manager.getAbnormal().remove();
						manager.getRuning().runding(ZhouZhuanXiangSaoMiao_qf.this, "数据加载中...");
		...
		public void handleMessage(Message msg) {
			...
			if (Mapplication.getApplication().ltQflkBoxNum.contains(box)) {
				if (null != box) {
					...
					System.out.println("//////////////" + S_application.getApplication().bianhao);
...
//				"list");
	...
	class SaomiaoThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				List<String> ltBox = RenwuData[0].get("zhouzhuanxiang").getValues();
	...
	public void onClick(View arg0) {
		...
		case R.id.qf_zhouzhuan_queding:
			manager.getRuning().runding(ZhouZhuanXiangSaoMiao_qf.this, "加载信息中...");
			queding.setEnabled(false);
