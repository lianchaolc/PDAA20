...
public class QingfenDengluByUserno extends Activity implements OnTouchListener {
	...
	public boolean onTouch(View view, MotionEvent even) {
		...
		if (MotionEvent.ACTION_UP == even.getAction()) {
			...
			// 取消
			case R.id.system_login_cancel:
				managerClass.getRuning().runding(QingfenDengluByUserno.this, "返回中...");
				login.setBackgroundResource(R.drawable.buttom_select_press);
				intent.putExtra("isOk", "error");
				QingfenDengluByUserno.this.setResult(QingfenDengluByUserno.this.RESULT_OK, intent);
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case -1:
				...
				managerClass.getResultmsg().resultmsg(QingfenDengluByUserno.this, S_application.wrong, false);
				break;
			case 0:
				managerClass.getRuning().remove();
				managerClass.getAbnormal().timeout(QingfenDengluByUserno.this, "登录超时,重试?", onclickreplace);
				break;
			case 1:
				managerClass.getRuning().remove();
				managerClass.getAbnormal().timeout(QingfenDengluByUserno.this, "网络连接失败,重试?", onclickreplace);
				break;
			case 2:
				managerClass.getRuning().remove();
				managerClass.getAbnormal().timeout(QingfenDengluByUserno.this, "请使用清分员或清分管理员身份!",
						new OnClickListener() {
							@Override
							public void onClick(View arg0) {
								editname.setText("");
			...
			case 3:
				...
				if (o_Application.left_user != null) {
					...
					if (o_Application.qingfen.getLoginUserName().equals(o_Application.left_user.getLoginUserName())) {
						managerClass.getAbnormal().timeout(QingfenDengluByUserno.this, "重复验证!", new OnClickListener() {
							@Override
							public void onClick(View arg0) {
								managerClass.getAbnormal().remove();
							}
