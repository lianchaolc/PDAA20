...
public class QingfenDengLuAcyivity extends BaseFingerActivity implements OnClickListener {
    // 指纹验证 左右姓名提示和底部提示
    ...
    public void getCharImgSucceed(byte[] charBytes, Bitmap img) {
        ...
        if (!firstSuccess && !"1".equals(f1)) {
            ShareUtil.finger_qingfen_denglu_left = img;
        } else {
            ShareUtil.finger_qingfen_denglu_right = img;
        }

        top_tishi.setText("获取指纹成功");
        if (isFlag) {
            isFlag = false;
            yanzhengFinger();
        }
    ...
    public void findFinger() {
        top_tishi.setText("正在获取指纹特征值");
    }

    @Override
    public void badCharHandler() {
        top_tishi.setText("获取指纹特征值失败,请重试");
    }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 0:
                    ...
                    manager.getAbnormal().timeout(QingfenDengLuAcyivity.this, "验证超时,重试?", OnClick1);
                    break;
                case 1:
                    isFlag = true;
                    manager.getRuning().remove();
                    manager.getAbnormal().timeout(QingfenDengLuAcyivity.this, "网络连接失败,重试?", OnClick1);
                    break;
                case 2:
                    isFlag = true;
                    // 第一位验证
                    if (!firstSuccess && !"1".equals(f1)) {
                        ...
                        o_Application.FingerQinfenLoginNum.add(result_user.getLoginUserId());
                        ...
                        Mapplication.getApplication().UserId = Mapplication.getApplication().user1.getYonghuZhanghao();
                        ...
                        if (right_name != null && right_name.equals(left_name)) {
                            ...
                            o_Application.FingerQinfenLoginNum.add(result_user.getLoginUserId());
                            img_right.setImageBitmap(ShareUtil.finger_qingfen_denglu_right);
                ...
                case 3:
                    ...
                    if (wrongleftNum >= ShareUtil.three) {
                        // 左侧跳用户登录
                        firstSuccess = false;
                        intent.setClass(QingfenDengLuAcyivity.this, QingfenDengluByUserno.class);
                        QingfenDengLuAcyivity.this.startActivityForResult(intent, 1);
                        top_tishi.setText("");
                        wrongleftNum = 0;
                ...
                case 4:
                    ...
                    if (xiaoleft.equals(xiaoright)) {// 如果2者的小组id相同则 跳转
                        Skip.skip(QingfenDengLuAcyivity.this, QingfenMenuActivity.class, null, 0);
                        QingfenDengLuAcyivity.this.finish();
                    } else {
                        manager.getAbnormal().timeout(QingfenDengLuAcyivity.this, "两清分员不在同一组不能进行下一步的业务",
                                new OnClickListener() {
                                    @Override
                                    public void onClick(View arg0) {
                                        manager.getAbnormal().remove();
    ...
    protected void onActivityResult(int arg0, int arg1, Intent arg2) {
        ...
        if (isOk.equals("success")) {
            if (!f1.equals("1")) {
                ...
                System.out.println(
                        "Mapplication.getApplication().user1=" + Mapplication.getApplication().user1.getLoginUserId());
                System.out.println("左边登录完成 o_Application.left_user：" + o_Application.left_user.getLoginUserName());

                // 取出左边用户登录的所属小组
                xiaoleft = o_Application.qingfen.getXiaozu() == null ? "" : o_Application.qingfen.getXiaozu();
            } else {
                // 判断验证 用户是否重复
                ...
                if (right_name != null && right_name.equals(left_name)) {
                    ...
                    System.out.println("Mapplication.getApplication().user2="
                            + Mapplication.getApplication().user2.getLoginUserName());

                    // 取出右边登录用的所属小组
                    xiaoright = o_Application.qingfen.getXiaozu() == null ? "" : o_Application.qingfen.getXiaozu();
    ...
    public void yanzhengFinger() {
        new Thread() {
            ...
            public void run() {
                ...
                System.out.println("QingfenDengLuAcyivity:" + GApplication.user);
                try {
                    System.out.println("GApplication.user.getOrganizationId()" + GApplication.user.getOrganizationId()
                            + "/id=" + GApplication.user.getLoginUserId());
                    result_user = yanzheng.checkFingerprint2(GApplication.user.getOrganizationId(),
