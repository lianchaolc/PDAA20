 ...
 * 
 */
public class QingFenJinDu_qf extends FragmentActivity implements OnClickListener {
		...
		public void handleMessage(Message msg) {
			...
			if (ltY.size() > 0) {
				for (int i = 0; i < ltY.size(); i++) {
					ylistData.add(new PeiSongDan(RenwuData[0].get("yiqingpeisong").get(i),
							RenwuData[0].get("suoshuwangdian").get(i)));
				}
			}
			if (ltW.size() > 0) {
				for (int i = 0; i < ltW.size(); i++) {
					wlistData.add(new PeiSongDan(RenwuData[1].get("weiqingpeisong").get(i),
							RenwuData[1].get("suoshuwangdian").get(i)));
				}
			}
	...
	private Handler timeoutHandle = new Handler() {// 连接超时handler
		public void handleMessage(Message msg) {
			manager.getRuning().remove();
			if (msg.what == 0) {
				manager.getAbnormal().timeout(QingFenJinDu_qf.this, "数据连接超时", new OnClickListener() {
					@Override
					public void onClick(View arg0) {
						getData();
						manager.getAbnormal().remove();
						manager.getRuning().runding(QingFenJinDu_qf.this, "数据加载中...");
					}
				});
			}
			if (msg.what == 1) {
				manager.getAbnormal().timeout(QingFenJinDu_qf.this, "网络连接失败", new OnClickListener() {
					@Override
					public void onClick(View arg0) {
						getData();
						manager.getAbnormal().remove();
	...
	class WeiQingAdapter extends BaseAdapter {
		...
		public View getView(int arg0, View arg1, ViewGroup arg2) {
			if (arg1 == null) {
				...
				wq.danhao = (TextView) arg1.findViewById(R.id.adapter_jihuadan_xianjin_juanbie);
				wq.wangdian = (TextView) arg1.findViewById(R.id.adapter_jihuadan_xianjin_count);
				arg1.setTag(wq);
			} else {
	...
	public void getData() {
		new Thread(new Runnable() {
			...
			public void run() {
				String params;
				try {
					params = new QingfenRenwuService().getParams(orderNum, "getQfschedule");
