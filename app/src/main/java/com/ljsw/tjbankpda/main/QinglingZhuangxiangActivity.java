...
public class QinglingZhuangxiangActivity extends FragmentActivity {
	...
	protected void onCreate(Bundle arg0) {
		...
		btnDizhi.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View arg0) {
				// manager.getRuning().runding(QinglingZhuangxiangActivity.this,
				// "正在开启RIFI,请稍等...");
		...
		btnTongji.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View arg0) {
				// manager.getResultmsg().resultOnlyMsg(QinglingZhuangxiangActivity.this,
				// "加载中...");
		...
		handler = new Handler() {
			public void handleMessage(Message msg) {
				...
				case 1:
					...
					// 加载默认的券别信息
					fragments.put("xianjin", new QinglingZhuangxiangXianjinFragment(quanbieXinxi));
					fm.beginTransaction().replace(R.id.fl_qingfen_zhuangxiang_context, fragments.get("xianjin"))
							.commit();
					break;

				case 2:
					// 获取超时
					manager.getAbnormal().timeout(QinglingZhuangxiangActivity.this, "数据连接超时", new OnClickListener() {
						@Override
						public void onClick(View arg0) {
				...
				case 3:
					// 获取失败
					manager.getAbnormal().timeout(QinglingZhuangxiangActivity.this, "获取券别信息失败", new OnClickListener() {
						@Override
						public void onClick(View arg0) {
							manager.getAbnormal().remove();
							getXQuanbieMingxi(); // 点击重新获取
