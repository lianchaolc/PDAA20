...
public class StringGetNum {
	...
	/**
	 * 计算尾号段 (此方法比较特殊，考虑到截取的数字如果前面为0会被int型抹去，此方法用比较填充的 方式将抹去的0补充回来)
	 * 
	 * @return
	 */
	public String jisuanweiHao(String beginHaoduan, int count) {
		String num = beginHaoduan; // 截取末尾数字字符串
		...
		int numInt = Integer.parseInt(num) + count - 1; // 计算出末尾的号段
		// String numString=numInt+""; //将末尾的号段转换成 字符串
		// int num2Length=numString.trim().length(); //计算末尾数字的长度
		/*
		 * if(numLength>num2Length){ //比较开始号段与末尾号段长度，如果末尾号段长度小于开始号段用"0"填充 String n="";
		 * int cha=numLength-num2Length; for(int i=0;i<cha;i++){ n+="0"; }
