...
public class QingLingChuKuJiaoJie_db extends FragmentActivity implements OnClickListener {
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 0:
				...
				manager.getAbnormal().timeout(QingLingChuKuJiaoJie_db.this, "验证超时,重试?", OnClick1);
				break;
			case 1:
				manager.getRuning().remove();
				manager.getAbnormal().timeout(QingLingChuKuJiaoJie_db.this, "网络连接失败,重试?", OnClick1);
				break;
			case 2:
				// 第一位验证
				if (!firstSuccess && !"1".equals(f1)) {
					...
					if (right_name != null && right_name.equals(left_name)) {
						...
						firstSuccess = false;
						manager.getRuning().runding(QingLingChuKuJiaoJie_db.this, "交接提交中...");
						JiaoJie();
					}
				}
				break;
			case 3:
				...
				if (wrongleftNum >= ShareUtil.three) {
					// 左侧跳用户登录
					firstSuccess = false;
					intent.setClass(QingLingChuKuJiaoJie_db.this, QingFenYuanDengLu.class);
					QingLingChuKuJiaoJie_db.this.startActivityForResult(intent, 1);
					top_tishi.setText("");
					wrongleftNum = 0;
			...
			case 4:
				...
				cashBoxNum = "";
				userId = "";
				Skip.skip(QingLingChuKuJiaoJie_db.this, RenWuLieBiao_db.class, null, 0);
				break;
			case 5:
				manager.getRuning().remove();
				manager.getAbnormal().timeout(QingLingChuKuJiaoJie_db.this, "交接超时,重试?", new OnClickListener() {
			...
			case 6:
				manager.getRuning().remove();
				manager.getAbnormal().timeout(QingLingChuKuJiaoJie_db.this, "网络连接失败,重试?", new OnClickListener() {
			...
			case 7:
				manager.getRuning().remove();
				manager.getAbnormal().timeout(QingLingChuKuJiaoJie_db.this, "提交失败,重试?", new OnClickListener() {
	...
	public void onClick(View arg0) {
		...
		case R.id.qingling_back:
			manager.getAbnormal().timeout(this, "退出将会清空数据!", new OnClickListener() {
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					...
					String isOk = new ZhouZhuanXiangJiaoJie().saveAuthLog(cashBoxNum, userId, userKuguanyuan, "21",
	...
	public void getcashBoxNumAnduserId() {
		...
		cashBoxNum = "";
		userId = "";
		System.out.println("-------------o_Application.numberlist.size()" + o_Application.numberlist.size());
