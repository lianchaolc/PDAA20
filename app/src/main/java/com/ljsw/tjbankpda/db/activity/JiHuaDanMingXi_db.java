...
public class JiHuaDanMingXi_db extends FragmentActivity implements OnClickListener {
	...
	public void getJiHuaMingXi() {
		...
		new Thread() {
			...
			public void run() {
				super.run();
				try {
					mingxi = new JiHuaDanMingXi().getZhuangxiangJihuadanDetail(o_Application.danhao.getJihuadan());
					if (!mingxi.equals("")) {
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 0:
				...
				manager.getAbnormal().timeout(JiHuaDanMingXi_db.this, "连接超时...", OnClick1);
				break;
			case 1:
				manager.getRuning().remove();
				manager.getAbnormal().timeout(JiHuaDanMingXi_db.this, "网络连接失败,重试?", OnClick1);
				break;
			case 2: // 调用接口成功
				manager.getRuning().remove();
...
//				new TurnListviewHeight(zhongkong_list);
			...
			case 3:
				manager.getRuning().remove();
				manager.getAbnormal().timeout(JiHuaDanMingXi_db.this, "获取数据失败...", new OnClickListener() {
	...
	public void onClick(View arg0) {
		...
		case R.id.jihuadan_dizhisaomiao:
			...
			if ((Integer.parseInt(mingxilist[2].get("dizhishu").getValues()
				...
				manager.getAbnormal().timeout(this, "需要扫箱数量与可扫码数不符,是否继续?", new OnClickListener() {

					@Override
					public void onClick(View v) {
						manager.getRuning().runding(JiHuaDanMingXi_db.this, "正在开启条形码扫描...");
						Log.i("起跳到验证指纹页面", "占时决定----");
						Skip.skip(JiHuaDanMingXi_db.this, QingLingChuKuJiaoJie_db.class, null, 0);
	...
	class XianJinAdapter extends BaseAdapter {
		...
		public View getView(int arg0, View arg1, ViewGroup arg2) {
			if (arg1 == null) {
				...
				viewXianJin.juanbie = (TextView) arg1.findViewById(R.id.adapter_jihuadan_xianjin_juanbie);
				viewXianJin.count = (TextView) arg1.findViewById(R.id.adapter_jihuadan_xianjin_count);
				arg1.setTag(viewXianJin);
			} else {
				viewXianJin = (ViewHolderXianJin) arg1.getTag();
			}
			viewXianJin.juanbie.setText(o_Application.jihuadan_list_xianjin.get(arg0).getJuanbie());
			viewXianJin.count.setText(o_Application.jihuadan_list_xianjin.get(arg0).getCount());
			return arg1;
		}
	...
	class ZhongKongAdapter extends BaseAdapter {
		...
		public View getView(int arg0, View arg1, ViewGroup arg2) {
			if (arg1 == null) {
				...
				viewzhongkong.pingzheng = (TextView) arg1.findViewById(R.id.adapter_jihuadan_zhongkong_type);
				viewzhongkong.count = (TextView) arg1.findViewById(R.id.adapter_jihuadan_zhongkong_count);
				arg1.setTag(viewzhongkong);
			} else {
				viewzhongkong = (ViewHolderZhongkong) arg1.getTag();
			}
			viewzhongkong.pingzheng.setText(o_Application.jihuadan_list_zhongkong.get(arg0).getPingzheng());
			viewzhongkong.count.setText(o_Application.jihuadan_list_zhongkong.get(arg0).getCount());
			return arg1;
		}
	...
	public void ZKSUM() {
		...
		if (o_Application.jihuadan_list_zhongkong.size() > 0) {
			for (int i = 0; i < o_Application.jihuadan_list_zhongkong.size(); i++) {
				zksum += Integer.parseInt(o_Application.jihuadan_list_zhongkong.get(i).getCount());
			}
		}
	}
	...
	public void getXianjin() {
		if (mingxilist[0].get("quanbie").getValues().size() > 0) {
			for (int i = 0; i < mingxilist[0].get("quanbie").getValues().size(); i++) {
				o_Application.jihuadan_list_xianjin
						.add(new JiHuaDan_Xianjin(mingxilist[0].get("quanbie").getValues().get(i),
	...
	public void getZhongkong() {
		if (mingxilist[1].get("zhongkongtype").getValues().size() > 0) {
			for (int i = 0; i < mingxilist[1].get("zhongkongtype").getValues().size(); i++) {
				o_Application.jihuadan_list_zhongkong
						.add(new JiHuaDan_Zhongkong(mingxilist[1].get("zhongkongtype").getValues().get(i),
								mingxilist[1].get("shuliang").getValues().get(i)));
			}
		}
	}
	...
	public void getDiZhiYaPin() {
		if (mingxilist[2].get("dizhibianhao").getValues().size() > 0) {
			for (int i = 0; i < mingxilist[2].get("dizhibianhao").getValues().size(); i++) {
				o_Application.jihuadan_list_dizhiyapin.add(mingxilist[2].get("dizhibianhao").getValues().get(i));
			}
		}
	}
