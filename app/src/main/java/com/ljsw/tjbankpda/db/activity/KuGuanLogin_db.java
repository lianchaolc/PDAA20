			...
			public void run() {
				...
				try {
					result_user = new YanZhengZhiWenService().checkFingerprint(
							...
							ShareUtil.ivalBack);
					if (result_user != null) {
						System.out.println("返回指纹用户：" + result_user.getLoginUserName());
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 0:
				...
				manager.getAbnormal().timeout(KuGuanLogin_db.this, "验证超时,重试?", OnClick1);
				break;
			case 1:
				manager.getRuning().remove();
				manager.getAbnormal().timeout(KuGuanLogin_db.this, "网络连接失败,重试?", OnClick1);
				break;
			case 2:
				// 第一位验证
				if (!firstSuccess && !"1".equals(f1)) {
					...
					o_Application.FingerLoginNum.add(result_user// 存放指纹登陆人的账号信息,
							.getLoginUserId());
					System.out.println("左侧验证完毕  保存帐号集合长度:" + o_Application.FingerLoginNum.size());
					...
					if (left_name != null && right_name.equals(left_name)) {
						...
						o_Application.FingerLoginNum.add(result_user.getLoginUserId());
						System.out.println("右侧验证完毕  保存帐号集合长度:" + o_Application.FingerLoginNum.size());
