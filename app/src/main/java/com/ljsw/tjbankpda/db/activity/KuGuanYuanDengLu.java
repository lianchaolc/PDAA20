	...
	public boolean onTouch(View view, MotionEvent even) {
		...
		if (MotionEvent.ACTION_UP == even.getAction()) {
			...
			case R.id.system_login_cancel:
				...
				KuGuanYuanDengLu.this.setResult(KuGuanYuanDengLu.this.RESULT_OK, intent);
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 0:
				...
				managerClass.getAbnormal().timeout(KuGuanYuanDengLu.this, "登录超时,重试?", onclickreplace);
				break;
			case 1:
				managerClass.getRuning().remove();
				managerClass.getAbnormal().timeout(KuGuanYuanDengLu.this, "网络连接失败,重试?", onclickreplace);
				break;

			case 2:
				managerClass.getRuning().remove();
				if (!lingshiUser.getLoginUserId().equals("4")
						&& !lingshiUser.getLoginUserId().equals(FixationValue.waibaoQingfenString)) {
					managerClass.getAbnormal().timeout(KuGuanYuanDengLu.this, "请使用库管员或清分管理员身份!", new OnClickListener() {
						@Override
						public void onClick(View arg0) {
							editname.setText("");
							editpwd.setText("");
					...
					if (!GApplication.loginJidouId.equals(lingshiUser.getOrganizationId())) {
						managerClass.getAbnormal().timeout(KuGuanYuanDengLu.this, "请使用本机构人员身份!", new OnClickListener() {
							@Override
							public void onClick(View arg0) {
								editname.setText("");
								editpwd.setText("");
						...
						if (o_Application.left_user != null) {
							if (lingshiUser.getLoginUserName().equals(o_Application.left_user.getLoginUserName())) {
								managerClass.getAbnormal().timeout(KuGuanYuanDengLu.this, "重复验证!",
										new OnClickListener() {
											@Override
											public void onClick(View arg0) {
												managerClass.getAbnormal().remove();
			...
			case 3:
				managerClass.getRuning().remove();
				managerClass.getAbnormal().timeout(KuGuanYuanDengLu.this, S_application.wrong, new OnClickListener() {
					@Override
					public void onClick(View arg0) {
						editname.setText("");
						editpwd.setText("");
