...
public class DiZhiYaPinSaoMiao_db extends FragmentActivity implements OnClickListener {
	...
	protected void onResume() {
		...
		SoundUtil.mContext = DiZhiYaPinSaoMiao_db.this;
		new Thread() {
			@Override
			public void run() {
				super.run();
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			for (int i = 0; i < o_Application.jihuadan_list_dizhiyapin.size(); i++) {
				if (o_Application.jihuadan_list_dizhiyapin.get(i).equals(number)) {
					aa = i;
					break;
				}
			}
...
//				new TurnListviewHeight(right);
	...
	public void onClick(View arg0) {
		...
		case R.id.dizhisaomiao_back:
			manager.getAbnormal().timeout(this, "退出将会清空已扫描数据!", new OnClickListener() {
		...
		case R.id.dizhisaomiao_chuku:
			...
			Skip.skip(DiZhiYaPinSaoMiao_db.this, QingLingChuKuJiaoJie_db.class, null, 0);
			break;
