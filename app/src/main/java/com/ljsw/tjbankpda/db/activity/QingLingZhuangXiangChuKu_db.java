...
public class QingLingZhuangXiangChuKu_db extends FragmentActivity implements OnClickListener, OnItemClickListener {
			...
			public void run() {
				...
				try {
					zhuangxiangJihuadan = new QingLingZhuangXiangChuKuLieBiao()
							.getZhuangxiangJihuadanList(o_Application.kuguan_db.getOrganizationId());
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 0:
				...
				manager.getAbnormal().timeout(QingLingZhuangXiangChuKu_db.this, "连接超时...", OnClick1);
				break;
			case 1:
				manager.getRuning().remove();
				manager.getAbnormal().timeout(QingLingZhuangXiangChuKu_db.this, "网络连接失败,重试?", OnClick1);
				break;
			case 2:
				manager.getRuning().remove();
				getData();
...
//				new TurnListviewHeight(listView);
			...
			case 3:
				manager.getRuning().remove();
				manager.getAbnormal().timeout(QingLingZhuangXiangChuKu_db.this, "没有任务", OnClick1);
				break;
			default:
				break;
			}
	...
	public void getData() {
		...
		if (jihuadan[0].get("jihuaxiang").getValues().size() > 0) {
			for (int i = 0; i < jihuadan[0].get("jihuaxiang").getValues().size(); i++) {
				ck = new QingLingZhuangXiangChuKu(jihuadan[0].get("jihuaxiang").getValues().get(i),
						jihuadan[0].get("riqi").getValues().get(i));
	...
	class MingXiAdapter extends BaseAdapter {
		LayoutInflater lf = LayoutInflater.from(QingLingZhuangXiangChuKu_db.this);
		ViewHodler view;
		...
		public View getView(int arg0, View arg1, ViewGroup arg2) {
			if (arg1 == null) {
				...
				view = new ViewHodler();
				view.danhao = (TextView) arg1.findViewById(R.id.adapter_renwumingxi_danhao);
				view.riqi = (TextView) arg1.findViewById(R.id.adapter_renwumingxi_riqi);
				arg1.setTag(view);
			} else {
				view = (ViewHodler) arg1.getTag();
			}
			view.danhao.setText(o_Application.qinglingchuku_jihuadan.get(arg0).getJihuadan());
			view.riqi.setText(o_Application.qinglingchuku_jihuadan.get(arg0).getRiqi());
			return arg1;
		}
