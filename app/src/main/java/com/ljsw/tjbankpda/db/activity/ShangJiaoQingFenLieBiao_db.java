...
public class ShangJiaoQingFenLieBiao_db extends FragmentActivity implements OnClickListener, OnItemClickListener {
...
//		new TurnListviewHeight(listview);
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 0:
				manager.getRuning().remove();
				manager.getAbnormal().timeout(ShangJiaoQingFenLieBiao_db.this, "连接超时...", OnClick1);
				break;
			case 1:
				manager.getRuning().remove();
				manager.getAbnormal().timeout(ShangJiaoQingFenLieBiao_db.this, "网络连接失败,重试?", OnClick1);
				break;
			case 2:
				manager.getRuning().remove();
				getData();
...
//				new TurnListviewHeight(listview);
			...
			case 3:
				manager.getRuning().remove();
				manager.getAbnormal().timeout(ShangJiaoQingFenLieBiao_db.this, "没有任务", new OnClickListener() {
	...
	class QingFenAdapter extends BaseAdapter {
		LayoutInflater lf = LayoutInflater.from(ShangJiaoQingFenLieBiao_db.this);
		ViewHodler view;
	...
	public void getData() {
		if (jihualist[0].get("jihuadan").getValues().size() > 0) {
			for (int i = 0; i < jihualist.length; i++) {
				 ...
				 * jihualist[i].get("xianluming").getValues()); System.out.println("shuliang:"+
				 * jihualist[i].get("shuliang").getValues()); System.out.println("xianluming:"+
