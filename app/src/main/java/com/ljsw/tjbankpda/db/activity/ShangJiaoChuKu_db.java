...
public class ShangJiaoChuKu_db extends FragmentActivity implements OnClickListener, OnItemClickListener {
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					...
					parms = new ShangJiaoChuKu().getShangjiaochukuMingxi(o_Application.kuguan_db.getOrganizationId());
					if (!parms.equals("")) {
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 0:
				manager.getRuning().remove();
				manager.getAbnormal().timeout(ShangJiaoChuKu_db.this, "加载超时,重试?", OnClick1);
				break;
			case 1:
				manager.getRuning().remove();
				manager.getAbnormal().timeout(ShangJiaoChuKu_db.this, "加载超时,重试?", OnClick1);
...
//				new TurnListviewHeight(listview);
			...
			case 3:
				manager.getRuning().remove();
				manager.getAbnormal().timeout(ShangJiaoChuKu_db.this, "无任务/获取任务失败,重试?", new OnClickListener() {
	...
	public void getData() {
		...
		/*
		 * public ShangJiaoChuku(String zongxianlu, String zongxuanluid, String
		 * zzxzongshu, List<String> zzxbianhao, List<String> zhixianlu, List<String>
		 * zzxshuliang) {
		 */
		...
		if (chukulist[0].get("zzxbianhao").getValues().size() > 0) {
			for (int i = 0; i < chukulist.length; i++) {
				o_Application.shangjiaochuku.add(new ShangJiaoChuku(chukulist[i].get("zongxianlu").getValues().get(0),
						chukulist[i].get("zongxianluId").getValues().get(0),
						chukulist[i].get("zzxzongshu").getValues().get(0), chukulist[0].get("zzxbianhao").getValues(),
						chukulist[0].get("zhixianlu").getValues(), chukulist[0].get("zzxshuliang").getValues()));
			}
	...
	class MingXiAdapter extends BaseAdapter {
		...
		public View getView(int arg0, View arg1, ViewGroup arg2) {
			...
			view.bianhao.setText(o_Application.shangjiaochuku.get(arg0).getZongxuanluid());
			view.xianlu.setText(o_Application.shangjiaochuku.get(arg0).getZongxianlu());
			view.count.setText(o_Application.shangjiaochuku.get(arg0).getZzxzongshu());
			return arg1;
		}
