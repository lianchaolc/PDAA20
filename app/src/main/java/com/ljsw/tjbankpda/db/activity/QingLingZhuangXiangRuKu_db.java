...
public class QingLingZhuangXiangRuKu_db extends FragmentActivity implements OnClickListener, OnItemClickListener {
...
//		new TurnListviewHeight(listview);
	...
	public void getRuKu() {
		manager.getRuning().runding(QingLingZhuangXiangRuKu_db.this, "数据加载中...");
		new Thread() {
			@Override
			public void run() {
				super.run();
				try {
					parms = new QingLingZhuangXiangRuKu().getQinglingRuku(o_Application.kuguan_db.getOrganizationId());
					if (!parms.equals("")) {
						System.out.print("params====" + parms);
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 0:
				...
				manager.getAbnormal().timeout(QingLingZhuangXiangRuKu_db.this, "加载超时,重试?", OnClick1);
				break;
			case 1:
				manager.getRuning().remove();
				manager.getAbnormal().timeout(QingLingZhuangXiangRuKu_db.this, "网络连接失败,重试?", OnClick1);
				break;
			case 2:
				manager.getRuning().remove();
				getData();
...
//				new TurnListviewHeight(listview);
			...
			case 3:
				manager.getRuning().remove();
				manager.getAbnormal().timeout(QingLingZhuangXiangRuKu_db.this, "没有任务", new OnClickListener() {
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
		...
		System.out.print(o_Application.qinglingruku.get(arg2).toString());
	...
	class QingLingAdapter extends BaseAdapter {
		LayoutInflater lf = LayoutInflater.from(QingLingZhuangXiangRuKu_db.this);
		ViewHodler view;
		...
		public View getView(int arg0, View arg1, ViewGroup arg2) {
			if (arg1 == null) {
				...
				view.danhao = (TextView) arg1.findViewById(R.id.adapter_qingling_bianhao);
				view.riqi = (TextView) arg1.findViewById(R.id.adapter_qingling_xianlu);
				view.count = (TextView) arg1.findViewById(R.id.adapter_qingling_count);
				arg1.setTag(view);
			} else {
				view = (ViewHodler) arg1.getTag();
			}
			view.danhao.setText(o_Application.qinglingruku.get(arg0).getDanhao());
	...
	public void getData() {
		if (rukulist[0].get("riqi").getValues().size() > 0) {
			for (int i = 0; i < rukulist.length; i++) {
				o_Application.qinglingruku.add(new QingLingRuKu(rukulist[i].get("jihuadan").getValues().get(0),
						rukulist[i].get("riqi").getValues().get(0), rukulist[i].get("zhouzhuanxiang").getValues()));
				System.out.println("-");
				/*
