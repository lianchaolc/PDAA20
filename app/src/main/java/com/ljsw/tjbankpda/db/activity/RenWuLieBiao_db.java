...
public class RenWuLieBiao_db extends FragmentActivity implements OnClickListener, OnItemClickListener {
    protected static final String TAG = "RenWuLieBiao_db";
...
//        manager.getRuning().runding(this, "数据加载中...");
        new Thread() {
            ...
            public void run() {
                super.run();
                try {
                    if (null == o_Application.kuguan_db.getOrganizationId() || o_Application.kuguan_db.getOrganizationId().equals("")) {
                        handler.sendEmptyMessage(5);
                    } else if (null == GApplication.user.getLoginUserId() || GApplication.user.getLoginUserId().equals("")) {
...
//				 setCount();
                ...
                case 5:
                    Toast.makeText(RenWuLieBiao_db.this,"机构id为空"+"数据没加载完成",500).show();
                    break;
                case 6:
                    Toast.makeText(RenWuLieBiao_db.this,"账号为空"+"数据没加载完成",500).show();
                    break;
                default:
                    break;
            }
...
//		}
    ...
    public void getData() {
        ...
        list.add(new RenWu("上缴清分出库", renwuinfo[0].get("shangjiaoqingfen").getValues().get(0)));// 为0 时报错
