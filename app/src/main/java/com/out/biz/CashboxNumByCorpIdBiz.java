...
public class CashboxNumByCorpIdBiz {
	...
	CashboxNumByCorpIdService getCashboxNumByCorpId() {
		return cashboxNumByCorpId = cashboxNumByCorpId == null ? new CashboxNumByCorpIdService() : cashboxNumByCorpId;
	}
	 ...
	 * @param planNum 计划编号
	 */
	public void gettCashboxNumByCorpId(String planNum, String corpId, String type) {
	...
	class Run implements Runnable {
		...
		public void run() {
			try {
				list = getCashboxNumByCorpId().getCashboxNumByCorpId(planNum, corpId, type);
				System.out.println("list----" + list.size());
				copy();
				if (list != null) {
					/**
					 * Modify begin by zhangxuewei at 01-DEC-2016 返回type到活动，用来区分加钞or回收
					 */
	...
	class Run2 implements Runnable {
		...
		public void run() {
			try {
				list = getCashboxNumByCorpId().getCashboxNumByATM(planNum, ATM, type);
				System.out.println("list----" + list.size());
				copy();
				if (list != null) {
					/**
					 * Modify begin by zhangxuewei at 12-DEC-2016 返回type到活动，用来区分加钞or回收
					 */
					Bundle data = new Bundle();
					data.putString("type", type);
					m.setData(data);
	...
	private void copy() {
		...
		WebJoin.list.clear();
		System.out.println("WebJoin.map_atm.size()----" + WebJoin.map_atm.size());
		...
		System.out.println("WebJoin.map_atm.size()2----" + WebJoin.map_atm.size());
	}
}
