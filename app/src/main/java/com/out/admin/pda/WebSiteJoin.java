...
public class WebSiteJoin extends Activity implements OnTouchListener {
	...
	CorpListByPlanNumBiz getCorpListByPlanNum() {
		return corpListByPlanNum = corpListByPlanNum == null ? new CorpListByPlanNumBiz() : corpListByPlanNum;
	}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (JoinResult.result) {
			...
			if (CashboxNumByCorpIdBiz.list != null && CashboxNumByCorpIdBiz.list.size() > 0) {
		...
		click = new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				managerClass.getRuning().runding(WebSiteJoin.this, "正在获取机构列表...");
				// 获取机构列表 参数:计划编号
				getCorpListByPlanNum().getCorpList(AssignBiz.order.getPlanNum(), type);
			}
		};
		...
		getCorpListByPlanNum().handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				case 1:
					// 把机构名称放入数组
					getarray();
					arrayad = new ArrayAdapter<String>(WebSiteJoin.this, R.layout.spinner_show, R.id.spinnertext,
							address);
					spinner.setAdapter(arrayad);

					break;
				case -1:
					managerClass.getAbnormal().timeout(WebSiteJoin.this, "连接异常！", click);
					break;
				case -4:
					managerClass.getAbnormal().timeout(WebSiteJoin.this, "连接超时！", click);
					break;
				case 0:
					managerClass.getSureCancel().makeSuerCancel(WebSiteJoin.this, "查找失败，没有数据",
		...
		click_getNum = new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if ("网点加钞钞箱交接".equals(addorback)) {
					managerClass.getRuning().runding(WebSiteJoin.this, "正在获取钞箱数量...");
					getCashboxNumByCorpId().gettCashboxNumByCorpId(AssignBiz.order.getPlanNum(), getCorpId(spinnerText),
							"0");
				} else if ("离行加钞钞箱交接".equals(addorback)) {
					managerClass.getRuning().runding(WebSiteJoin.this, "正在获取钞箱数量...");
					getCashboxNumByCorpId().gettCashboxNumByATM(AssignBiz.order.getPlanNum(), getCorpId(spinnerText),
							"0");
		...
		getCashboxNumByCorpId().handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				case 1:
					/**
					 * Modify begin by zhangxuewei at 01-DEC-2016 type到活动，用来区分加钞or回收
					 */
					if (type != null && type.equals("0")) {
						needNum.setText(Integer.parseInt(CashboxNumByCorpIdBiz.boxCount) + "");
					} else {
				...
				case 0:
					...
					break;
				case -1:
					managerClass.getAbnormal().timeout(WebSiteJoin.this, "连接异常，要重试吗？", click_getNum);
					break;
				case -4:
					managerClass.getAbnormal().timeout(WebSiteJoin.this, "连接超时，要重试吗？", click_getNum);
					break;
				}
		...
		spinner.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> parent, View view, int position, long arg3) {
				...
				if (!"请选择地点".equals(spinnerText) && !"请选择ATM机".equals(spinnerText)) {
					boolean isOK = false;
					// 根据所在位置获取交接和加钞钞箱数量 参数：计划编号，机构ID
					if ("网点加钞钞箱交接".equals(addorback)) {
						managerClass.getRuning().runding(WebSiteJoin.this, "正在获取钞箱数量...");
						getCashboxNumByCorpId().gettCashboxNumByCorpId(AssignBiz.order.getPlanNum(),
						...
						managerClass.getRuning().runding(WebSiteJoin.this, "正在获取钞箱数量...");
						getCashboxNumByCorpId().gettCashboxNumByATM(AssignBiz.order.getPlanNum(),
	...
	class Ad extends BaseAdapter {
		...
		public View getView(int arg0, View view, ViewGroup arg2) {
			...
			if (v == null) {
				...
				v = GolbalView.getLF(WebSiteJoin.this).inflate(R.layout.boxinformation_item, null);
				holder.boxNum = (TextView) v.findViewById(R.id.box_num);
				holder.btndel = (Button) v.findViewById(R.id.webjoin_del);
				holder.detele = (TextView) v.findViewById(R.id.newstop_item_del);
				holder.cancel = (TextView) v.findViewById(R.id.newstop_item_cancel);
				holder.brand = (TextView) v.findViewById(R.id.brand);
				holder.lmenu = (LinearLayout) v.findViewById(R.id.newstop_item_menu);
			...
			v.setOnTouchListener(new OnTouchListener() {
				...
				public boolean onTouch(View view, MotionEvent even) {
					...
					if (MotionEvent.ACTION_DOWN == even.getAction()) {
						...
						holder.boxNum.setBackgroundResource(R.color.gray_msg_bg);
					}
					// 手指松开的时候
					if (MotionEvent.ACTION_UP == even.getAction()) {
						holder.brand.setBackgroundResource(R.color.transparency);
						holder.boxNum.setBackgroundResource(R.color.transparency);
					}
	...
	public boolean onTouch(View view, MotionEvent even) {
		...
		if (MotionEvent.ACTION_UP == even.getAction()) {
			...
			case R.id.webatminfo:
				...
				if ("网点加钞钞箱交接".equals(addorback) || "离行加钞钞箱交接".equals(addorback)) {
					 ...
					 * BankDoublePersonLogin.class, bundle, managerClass.getGolbalutil().ismover);
					 */

					managerClass.getGolbalutil().gotoActivity(WebSiteJoin.this, BankDoublePersonLogin.class, bundle,
							managerClass.getGolbalutil().ismover);

				} else if ("网点回收钞箱交接".equals(addorback) || "离行回收钞箱交接".equals(addorback)) {
					bundle.putString("businName", "网点回收钞箱交接");
					bundle.putString("type", "05");
					managerClass.getGolbalutil().gotoActivity(WebSiteJoin.this, SupercargoJoin.class, bundle,
							managerClass.getGolbalutil().ismover);
				}
	...
	protected void onStart() {
		...
		try {
			...
			managerClass.getGolbalView().toastShow(WebSiteJoin.this, "请再次获取派工单");
	...
	public void check() {
		 ...
		 */
		String flag = ch.checkBox(WebJoin.list);
		if (WebJoin.list != null && flag.equals("00") && count > 0 && count == WebJoin.list.size()) {
			/** end */
			...
			Log.i("检测不通过", "检测不通过");
			managerClass.getGolbalView().toastShow(WebSiteJoin.this, "请检测钞箱数量及有效性");
		}
