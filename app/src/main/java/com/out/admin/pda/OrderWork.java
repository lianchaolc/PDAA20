...
//		上面名字不显示
		...
		clcik = new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				managerClass.getRuning().runding(OrderWork.this, "正在获取派工单信息...");
				// 重新请求数据
		...
		getAssign().handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				case 1:
					...
					break;
				case -1:
					managerClass.getAbnormal().timeout(OrderWork.this, "连接异常，重新连接？", clcik);
					break;
				case 0:
					orderwork.setVisibility(View.GONE);
					tastdata.setVisibility(View.VISIBLE);
					break;
				case -4:
					managerClass.getAbnormal().timeout(OrderWork.this, "连接超时，重新连接？", clcik);
					break;
				}
		...
		if (MotionEvent.ACTION_UP == even.getAction()) {
			...
			case R.id.webadd:
				...
				if (!webaddnum.getText().equals("0")) {
					...
					managerClass.getGolbalutil().gotoActivity(OrderWork.this, WebSiteJoin.class, bundle,
							managerClass.getGolbalutil().ismover);
				} else {
					managerClass.getGolbalView().toastShow(OrderWork.this, "没有任务");
				}
			...
			case R.id.aboutjoin:
				...
				if (AssignBiz.order == null) {
					Toast.makeText(OrderWork.this, "暂时没有情况可查看", Toast.LENGTH_SHORT).show();
				} else {
					managerClass.getGolbalutil().gotoActivity(OrderWork.this, AboutJoin.class, bundle,
							managerClass.getGolbalutil().ismover);
				}
			...
			case R.id.orderwork:
				...
				managerClass.getGolbalutil().gotoActivity(OrderWork.this, OrderWorkInformation.class, null,
						managerClass.getGolbalutil().ismover);
				break;
			// 刷新
			case R.id.order_refresh:
	...
	public boolean onMenuOpened(int featureId, Menu menu) {
		...
		MenuShow.pw.showAtLocation(findViewById(R.id.orderwork_box), Gravity.BOTTOM, 0, 0);
		return false;
	}
	...
	private Handler leixingHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 1:
				...
				// 获取派工单列表，参数：用户ID
				Log.i("login_username", GApplication.getApplication().app_hash.get("login_username").toString());
				if ("在行式".equals(type)) {
					getAssign().getAssig(GApplication.user.getYonghuZhanghao(), "0");
					webadd.setText("网点加钞钞箱交接");
					webback.setText("网点回收钞箱交接");
			...
			case -1:
				managerClass.getAbnormal().timeout(OrderWork.this, "查询异常，重新查询？", clcik);
				break;
			case 3:
				managerClass.getAbnormal().timeout(OrderWork.this, "查询超时，重新查询？", clcik);
				break;
